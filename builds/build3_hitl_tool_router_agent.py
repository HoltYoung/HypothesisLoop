...
=============================================================
QAC387 | Holt Young & Sam Penn | Spring 2026

A CLI agent that analyzes the UCI Adult Income dataset using a
human-in-the-loop (HITL) workflow with intelligent tool routing.

The top-level ROUTER decides whether to:
    (A) run one of the Build0 analysis tools from the allow-list, OR
    (B) fall back to CodeGen + optional Execute (subprocess).

This agent is part of the HypothesisLoop project — an LLM-powered
iterative hypothesis-testing system for data analysis.

Commands:
    ask <request>   — ROUTER decides tool-run vs codegen (HITL)
    tool <request>  — force tool mode (HITL)
    code <request>  — force codegen mode (HITL)
    run             — execute last approved script
    suggest <q>     — ask questions about the dataset (LLM)

Run:
  python builds/build3_hitl_tool_router_agent.py --data data/adult.csv --report_dir reports --tags build3 --memory

  Add --stream to stream LLM output.
"""
...
def build_router_chain(
    ...
    {{"mode":"tool","tool":"plot_histograms","args":{
        "numeric_cols":["age","hours_per_week"]},"note":"<brief>"}}
    ```
    ...
    - Correlations -> pearson_correlation (NOT plot_corr_heatmap, which needs a pre-computed matrix)
    ...
    CRITICAL tool-specific argument rules (use EXACT parameter names):
    ...
    - pearson_correlation: no required args (runs on all numeric columns)
    - plot_histograms: args={{"numeric_cols":["<col1>","<col2>"]}}

    Examples:
    User: "frequency table for education"
    ```json
    {{"mode":"tool","tool":"summarize_categorical","args":{{"column":"education"}},"note":"Frequency table is a categorical summary."}}
    ```

    User: "frequency tables for sex and race"
    ```json
    {{"mode":"tool","tool":"summarize_categorical","args":{{"cat_cols":["sex","race"]}},"note":"Summarize multiple categorical columns."}}
    ```
    ...
    User: "histograms for age and hours_per_week"
    ```json
    {{
        "mode":"tool",
    ...
    User: "what is the average income by education level"
    ```json
    {{"mode":"codegen","code_request":"Group by education level and compute mean of education_num, hours_per_week, and income distribution. Create a summary table and bar chart.","note":"No single tool handles grouped aggregation with custom metrics."}}
    ```
""")
...
Examples (Adult Income dataset):
  ask run a frequency table for education
  ask fit a regression of hours_per_week on age and education_num
  ask show histograms for age and capital_gain
  tool run a correlation heatmap for numeric columns
  code create a grouped bar chart of income by sex and race
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build 3: HITL + Tool Router Agent — UCI Adult Income Analysis (Langfuse-traced)"
    )
    ...
    print("\n=== Build 3: HITL + Tool Router Agent — Adult Income Analysis ===\n")
