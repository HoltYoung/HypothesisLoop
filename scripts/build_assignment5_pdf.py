"""Render the Assignment 5 testing & validation log as a PDF in the same
visual style as scripts/build_tracing_log_pdf.py.
...
# --------------------------------------------------------------------------------------
# Styles (mirrors build_tracing_log_pdf.py for visual parity with Build 4 PDF)
# --------------------------------------------------------------------------------------
def build_styles():
...
def cell_table_style(font_size=8.5):
    """For tables with Paragraph cells — top-aligned for wrapping."""
    return TableStyle(
        [
            ("BACKGROUND", (0, 0), (-1, 0), HEADER_BG),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
...
def make_sweep_table(rows):
    """rows = list of [#, prompt, category, route, R, A, X, Q, notes]."""
...
def make_scorecard_table():
    ...
    table = Table(
        cells,
        colWidths=[2.4 * inch, 0.65 * inch, 1.0 * inch, 1.0 * inch, 1.05 * inch, 0.40 * inch],
        repeatRows=1,
    )
    table.setStyle(cell_table_style())
    return table
...
def build_story():
    ...
    s.append(Paragraph(
        ...
        "Tool Router Agent against the UCI Adult Income dataset. The goal — per the "
        "assignment brief — is not to demonstrate a perfect agent but to identify where "
        "the agent works and where it fails, so we know what to improve next.",
        styles["body"],
    ))
    ...
    s.append(Paragraph(
        ...
        "bad-input, unrelated) on <b>two providers</b>:",
        styles["body"],
    ))
    s.append(make_provider_table())
    s.append(Spacer(1, 0.10 * inch))
    s.append(Paragraph(
        "The Build 4 agent was extended with a <font face='Courier'>--provider {openai,moonshot}</font> "
        "flag during this assignment so the same prompts could be compared across model "
        "families without changing the agent's prompts, RAG index, or HITL flow. Embeddings "
        "continue to use <font face='Courier'>text-embedding-3-small</font> regardless of "
        "chat provider. All routing, retrieval, tool, and codegen calls are traced in "
        "Langfuse Cloud under their respective session IDs.",
        styles["body"],
    ))
    s.append(PageBreak())
    ...
    s.append(Paragraph(
        ...
        "executed cleanly, <b>Q</b> = response quality (1–5, 0 = crash).",
        styles["caption"],
    ))
    ...
    s.append(Paragraph(
        "Six distinct failure modes surfaced across the 16 test runs. The first two "
        "are critical — they reproduce on both providers and degrade correctness, not "
        "just polish. The remaining four are quality issues.",
        styles["body"],
    ))
    ...
    s.append(Paragraph(
        "Targets are defined in the assignment brief. Router judgment is the agent's "
        "standout strength; everything downstream of the router fails to clear the bar.",
        styles["body"],
    ))
    ...
    s.append(Spacer(1, 0.10 * inch))
    s.append(Paragraph(
        "<b>Conclusion.</b> Upgrading the model fixes some second-order issues (arg quality, "
        "refusal phrasing) but does not fix the first-order bugs (F1, F2). Those require "
    ...
    s.append(Paragraph(
        "Router judgment was the standout strength — both <font face='Courier'>gpt-4o-mini</font> "
        "and <font face='Courier'>kimi-k2.6</font> produced defensible routing decisions on "
        "100% of prompts (16/16). The router correctly distinguished tool requests from "
        "codegen requests, recognized when a knowledge-base question wasn't an analysis "
        "(\"when should I use multiple regression?\"), recognized off-scope prompts (\"how do "
        "I fix my kitchen sink?\"), and flagged a non-existent variable. Langfuse "
        "instrumentation was also robust: every router/retrieval/tool/codegen/summary step "
        "shows up cleanly under the session ID, making it easy to audit any single prompt "
        "across providers.",
        styles["body"],
    ))
    ...
    s.append(Paragraph(
        "Routing was good; the pipeline downstream of routing was broken. The single "
        "most damaging issue was the validator that rejects <font face='Courier'>mode:\"answer\"</font> "
        "even though the router system prompt explicitly lists it as a legal mode. This "
        "dead branch crashed 4 of 8 prompts on OpenAI and 5 of 8 on Kimi — every "
        "conceptual question, every off-scope refusal, and every bad-input flag was "
        "generated correctly by the router and then thrown away by the validator. "
        "The second issue was the codegen chain producing code unrelated to the user "
        "request (e.g., responding to \"bin age into 5 quantiles\" with a \"boxplot of "
        "hours_per_week by income\"). Both bugs reproduce identically on a much stronger "
        "model, so they are not model-quality bugs — they are contract bugs between the "
        "router prompt and the agent's downstream code.",
        styles["body"],
    ))

    s.append(Paragraph("What is the single highest-priority improvement?", styles["h2"]))
    s.append(Paragraph(
        "Fix the router-output contract. Specifically: (a) accept "
        "<font face='Courier'>mode:\"answer\"</font> in the validator and route it through "
        "a small \"answer\" handler that prints the router's note/response text to the user "
        "(no tool call, no codegen, no HITL gate); and (b) audit the codegen chain to "
        "confirm it is being passed <font face='Courier'>user_request</font> and not some "
        "stale or misrouted variable, since the symptom (always producing \"exploratory "
        "overview\" code) suggests it is not seeing the actual prompt. Both fixes are "
        "localized, take an hour, and would lift the success-criteria scorecard from a "
        "fail on most rows to a pass on most rows. Improvements to retrieval (F6) and "
        "arg-fabrication (F3) are real but secondary — they degrade quality, while the "
        "validator and codegen bugs degrade correctness.",
        styles["body"],
    ))
    ...
    s.append(Paragraph(
        "OpenAI sweep (local):", styles["body"],
    ))
    s.append(Paragraph(
